...
/// \brief function which is called for each facet when iterating through a
/// given forest
...
void pXest_interface_list_fn(p4est_iter_face_info_t *info, void *user_data) {
  ...
  InterfaceList *list = static_cast< InterfaceList * >(user_data);
   ...
   * */
  LOG_DEBUG(1, " interface: num_sides "
                   << num_sides << " orientation "
                   << static_cast< int16_t >(orientation)
                   << " is_tree_boundary "
                   << static_cast< int16_t >(is_tree_boundary));
  ...
  for (int js = 0; js < num_sides; ++js) {
    ...
    if (is_hanging[js] != 0) {
      ...
      for (int jq = 0; jq < num_hanging_cells; jq++) {
        ...
        is_ghost[js][jq] =
            static_cast< int16_t >(side[js]->is.hanging.is_ghost[jq]);

        // ****
        /*
        p4est_locidx_t quadid = side[js]->is.hanging.quadid[jq];
        QuadData* q_ptr = pXest_get_quad_data_ptr(quad);
        std::cout << "   Quad " << jq << " in current side has local id " <<
        quadid
  ...
  if (num_sides == 1) // Add boundary facet
  {
    ...
    LOG_DEBUG(1, "side " << 0 << " tree id " << side[0]->treeid << " face (db)"
                         << master_facet_number << " is hanging "
                         << is_hanging[0]);
    ...
    if (is_hanging[0] + is_hanging[1] == 0) // standard interior facet
    {
      ...
      Interface &interface =
          list->add_interface(cell_indices[master], db_facet_nr[master]);
      interface.add_slave(cell_indices[slave], db_facet_nr[slave]);

      LOG_DEBUG(1,
                "(Regular facet) Master cell index: " << cell_indices[master]);
    ...
    } else // facet with hanging node
    {
      ...
      int master_facet_number =
          pXest_facet_number_pXest_to_db(pXest_facet_nr[master_side]);
      ...
      for (int jq = 0; jq < num_hanging_cells; ++jq) {
        ...
        int slave_facet_number =
            pXest_facet_number_pXest_to_db(pXest_facet_nr[slave_side]);
...
/// \brief function which is called on each local quadrant when calling
/// p4est_refine(). The return value of this function decides whether the
/// quadrant should be refined
...
int pXest_refine_decide_fn(p4est_t *forest, p4est_topidx_t tree,
...
int pXest_refine_decide_fn(p8est_t *forest, p4est_topidx_t tree,
...
/// \brief function that is called for each children quadrant which is generated
/// when calling p4est_refine(). The purpose of this function is to set
/// initialize the QuadData structure for the newly created quadrants.
/// @param[in] forest pointer to underlying forest object
/// @param[in] tree_id global id of current tree object
/// @param[in] quad pointer to current quadrant object

void pXest_refine_init_fn(p4est_t *forest, p4est_topidx_t tree_id,
                          p4est_quadrant_t *quad) {
...
void pXest_refine_init_fn(p8est_t *forest, p4est_topidx_t tree_id,
                          p8est_quadrant_t *quad) {
  // initialize quad data for new quadrant
  QuadData *q_ptr = new QuadData();
  quad->p.user_data = q_ptr;
...
/// \brief function which is called on each parent and all its children
/// quadrants in p4est_refine after p4est_refine_init_fn() is called for the
/// children quadrants. The purpose of this function is to copy the data from
/// parent quadrant to its children quadrants
/// @param[in] forest pointer to underlying forest object
/// @param[in] which_tree global id of current tree object
/// @param[in] num_outgoing number of quadrants to be removed when refining a
/// given quadrant
...
void pXest_refine_replace_fn(p4est_t *forest, p4est_topidx_t which_tree,
  ...
  ForestRefineData *i_ptr =
      static_cast< ForestRefineData * >(forest->user_pointer);
  ...
  for (int jq = 0; jq < num_incoming; ++jq) {
    ...
    LOG_DEBUG(
        3, " parent index "
               << parent_index << " parent x "
               << outgoing[0]->x *
                      static_cast< int >(
                          std::pow(static_cast< double >(2), -P4EST_MAXLEVEL))
               << " parent y "
               << outgoing[0]->y *
                      static_cast< int >(
                          std::pow(static_cast< double >(2), -P4EST_MAXLEVEL))
               << " own x "
               << cur_quad->x * static_cast< int >(std::pow(
                                    static_cast< double >(2), -P4EST_MAXLEVEL))
               << " own y "
               << cur_quad->y * static_cast< int >(std::pow(
                                    static_cast< double >(2), -P4EST_MAXLEVEL))
               << " p4est sibling id " << p4est_sibling << " hiflow sibling id "
               << hiflow_sibling << " cell id " << own_id
    ...
    LOG_DEBUG(
        3, " parent index "
               ...
               << cur_quad->y * static_cast< int >(std::pow(
                                    static_cast< double >(2), -P4EST_MAXLEVEL))
               << " own z "
               << cur_quad->z * static_cast< int >(std::pow(
                                    static_cast< double >(2), -P4EST_MAXLEVEL))
               << " p4est sibling id " << p4est_sibling << " hiflow sibling id "
               << hiflow_sibling << " cell id " << own_id
               << " owner rank: " << forest->mpirank);
...
/// \brief Function which is called for each coarsenbale family of quadrants.
/// This function decides whether the given family should be coarsened
/// @param[in] forest pointer to underlying forest object
/// @param[in] tree global id of current tree object
/// @param[in] quads pointers to quadrants forming a family
/// @return true if family of quads shouzld be coarsened

int pXest_coarsen_decide_fn(p4est_t *forest, p4est_topidx_t tree,
                            p4est_quadrant_t *quads[]) {
  // get pointer to external user data
  ForestCoarsenData *i_ptr =
      static_cast< ForestCoarsenData * >(forest->user_pointer);
  ...
  if (num_markers <= -4) { // yes
    LOG_DEBUG(2, "Tree " << tree << " num_coarsen_markers " << num_markers
                         << " -> coarsen");
...
int pXest_coarsen_decide_fn(p8est_t *forest, p4est_topidx_t tree,
...
/// \brief Function which is called for each quadrant that is created when
/// coarsening a faily of quadrants. Here, the user data structure is
/// initialized
/// @param[in] forest pointer to underlying forest object
/// @param[in] tree_id global id of current tree object
/// @param[in] quad pointer to current quadrant object

void pXest_coarsen_init_fn(p4est_t *forest, p4est_topidx_t tree_id,
                           p4est_quadrant_t *quad) {
...
void pXest_coarsen_init_fn(p8est_t *forest, p4est_topidx_t tree_id,
                           p8est_quadrant_t *quad) {
  // init user data for new quadrant
  QuadData *q_ptr = new QuadData();
  quad->p.user_data = q_ptr;
...
/// \brief function which is called on each parent and all its children
/// quadrants in p4est_coarsen after p4est_coarsen_init_fn() is called for the
/// parent quadrants. The purpose of this function is to copy the data from
/// chidlren quadrants to their parent quadrant
/// @param[in] forest pointer to underlying forest object
/// @param[in] which_tree global id of current tree object
/// @param[in] num_outgoing number of quadrants to be removed when coarsening a
/// given family
/// @param[in] outgoing pointers to quadrants to be removed (i.e. coarsened)
/// @param[in] num_incoming number of newly created quadrant (parent of removed
/// quadrants)
...
/// \brief Function which is called for each coarsenbale family of quadrants.
/// This function decides whether the given family should be coarsened. Here the
/// return value is always false, since this function is used to track all
/// coarsenable families of quadrants
/// @param[in] forest pointer to underlying forest object
/// @param[in] tree global id of current tree object
/// @param[in] quads pointers to quadrants forming a family
/// @return always false

int pXest_coarsen_find_fn(p4est_t *forest, p4est_topidx_t tree,
                          p4est_quadrant_t *quads[]) {
  // get pointer to external user data
  ForestPatchData *i_ptr =
      static_cast< ForestPatchData * >(forest->user_pointer);
...
int pXest_coarsen_find_fn(p8est_t *forest, p4est_topidx_t tree,
                          p8est_quadrant_t *quads[]) {
  // get pointer to external user data
  ForestPatchData *i_ptr =
      static_cast< ForestPatchData * >(forest->user_pointer);
...
/// \brief function that is called for each children quadrant which is generated
/// when calling p4est_balance(). The purpose of this function is to set
/// initialize the QuadData structure for the newly created quadrants.
/// @param[in] forest pointer to underlying forest object
/// @param[in] tree_id global id of current tree object
/// @param[in] quad pointer to current quadrant object

void pXest_balance_init_fn(p4est_t *forest, p4est_topidx_t tree_id,
                           p4est_quadrant_t *quad) {
  // initialize quad data for new quadrant
  QuadData *q_ptr = new QuadData();
  quad->p.user_data = q_ptr;
...
void pXest_balance_init_fn(p8est_t *forest, p4est_topidx_t tree_id,
                           p8est_quadrant_t *quad) {
  // initialize quad data for new quadrant
  QuadData *q_ptr = new QuadData();
  quad->p.user_data = q_ptr;
...
void pXest_balance_replace_fn(p4est_t *forest, p4est_topidx_t which_tree,
  ...
  // get pointer to external user data
  ForestBalanceData *i_ptr =
      static_cast< ForestBalanceData * >(forest->user_pointer);

  // get data and index of parent
  // Note: in case of multiple refinement, this data always corresponds to the
  // initial quadrant to be refined in the following called 'ancestor'
  QuadData *ancestor_ptr = pXest_get_quad_data_ptr(outgoing[0]);
  assert(ancestor_ptr != nullptr);
  ...
  for (int jq = 0; jq < num_incoming; ++jq) {
    ...
    // add newly created quadrants with ancestor given by ancestor_index to user
    // data
    ...
    std::map< EntityNumber, std::vector< p4est_quadrant_t > >::iterator it_q =
        i_ptr->new_quads4.find(ancestor_index);
    if (it_q == i_ptr->new_quads4.end()) {
      // First refinement of cell with ancestor_index
      i_ptr->new_quads4.insert(
          std::pair< EntityNumber, std::vector< p4est_quadrant_t > >(
              ancestor_index, new_quad));
      number_desc_quads = 1;
    } else {
    ...
    LOG_DEBUG(
        2,
        " ancestor index "
            << ancestor_index << " on process " << forest->mpirank
            << " parent x "
            << outgoing[0]->x /** static_cast < int > ( std::pow ( static_cast <
                                 double > ( 2 ), -P4EST_MAXLEVEL ) )*/
            << " parent y "
            << outgoing[0]->y /** static_cast < int > ( std::pow ( static_cast <
                                 double > ( 2 ), -P4EST_MAXLEVEL ) )*/
            << " parent level "
            << static_cast< int >(outgoing[0]->level)
            //<< " own x " << cur_quad->x * static_cast < int > ( std::pow (
            // static_cast < double > ( 2 ), -P4EST_MAXLEVEL ) )
            //<< " own y " << cur_quad->y * static_cast < int > ( std::pow (
            // static_cast < double > ( 2 ), -P4EST_MAXLEVEL ) )
            << " x " << cur_quad->x << " y " << cur_quad->y << " own level "
            << static_cast< int >(cur_quad->level) << " own level "
            << static_cast< int >(incoming[jq]->level)
...
void pXest_balance_replace_fn(p8est_t *forest, p4est_topidx_t which_tree,
  ...
  for (int jq = 0; jq < num_incoming; ++jq) {
    ...
    int number_desc_quads;
    std::map< EntityNumber, std::vector< p8est_quadrant_t > >::iterator it_q =
        i_ptr->new_quads8.find(ancestor_index);
    if (it_q == i_ptr->new_quads8
                    .end()) { // First refinement of cell with ancestor_index
      i_ptr->new_quads8.insert(
          std::pair< EntityNumber, std::vector< p8est_quadrant_t > >(
              ancestor_index, new_quad));
      number_desc_quads = 1;
    } else { // Repeated refinement of cell with ancestor_index
      it_q->second.push_back(new_quad[0]);
      number_desc_quads = it_q->second.size();
    }
...
MeshPXest::MeshPXest(TDim tdim, GDim gdim, MeshPXestDatabasePtr db,
                     const std::vector< Id > &cells, int history_index,
                     const std::vector< bool > &cell_is_local,
                     std::vector< MasterSlave > period, bool is_rectangular,
                     std::map< int, ConstMeshPtr > const *mesh_history)
    : MeshDbView(tdim, gdim, db, cells, period, is_rectangular) {
  assert(history_index >= 0);
  assert(cells.size() == cell_is_local.size() || cell_is_local.empty());

  LOG_DEBUG(1,
            "create mesh " << this << " with history index " << history_index);
  this->pXest_db_ = db;
  this->history_index_ = history_index;

  // If db contains forest: Set cell indices for local quadrants, without ghost
  // cells
  ...
  if (cell_is_local.empty()) {
    this->cell_is_local_.resize(cells.size(), true);
  } else {
    this->cell_is_local_ = cell_is_local;
  }

  if (mesh_history != nullptr) {
    std::map< int, ConstMeshPtr >::const_iterator it;
    for (it = mesh_history->begin(); it != mesh_history->end(); it++) {
      this->mesh_history_[it->first] = it->second;
    }
  }
  this->connection_mode_ = 0;
  this->patch_mode_ = false;
  this->mesh_impl_ = IMPL_P4EST;
}

MeshPXest::MeshPXest(TDim tdim, GDim gdim, std::vector< MasterSlave > period,
                     bool is_rectangular)
    : MeshDbView(tdim, gdim, period, is_rectangular) {
  LOG_DEBUG(1, "create mesh " << this << " with history index " << -1);
...
MeshPXest::~MeshPXest() {
  LOG_DEBUG(1, "delete mesh " << this << " of history index "
                              << this->history_index_);
  this->cell_id_to_index_.clear();
  this->cell_is_local_.clear();
  this->db_.reset();
  this->pXest_db_.reset();
};
...
MeshPtr MeshPXest::refine(std::vector< EntityNumber > &refinements) const {
  ...
  if (tdim == 2) {
      ...
      for (int jq = 0; jq < num_quads; ++jq) {
        ...
        if (index >= 0) {
          ...
          if (r > 0) {
            ...
            LOG_DEBUG(2, "[" << forest->mpirank << "] Refining cell " << index
                             << " with id " << cell_id << " with refinement "
                             << 0);

            // refine cell entity
            const RefinementTree *reftree =
                current_cell.cell_type().refinement_tree(0);
            std::vector< int > local_sub_cell_numbers;
            const std::vector< Id > children_cells =
                this->compute_refined_cells(current_cell, reftree,
                                            local_sub_cell_numbers);
            for (size_t l = 0; l < children_cells.size(); ++l) {
            ...
            children_ids_per_parent[index].insert(
                children_ids_per_parent[index].end(), children_cells.begin(),
                children_cells.end());
          }
          if (r == 0) {
            // keep cell
            LOG_DEBUG(2, "[" << forest->mpirank << "] Keep cell " << index
                             << " with id " << cell_id);
          ...
          if (r < 0) {
            // mark for coarsening
            LOG_DEBUG(2, "[" << forest->mpirank << "] Mark cell " << index
                             << " with id " << cell_id << " for coarsening "
                             << r);
            new_cells.find_insert(cell_id);
            new_cell_is_local[cell_id] = true;
            q_ptr->set_coarsen(r);
    ...
    // Loop over all ghost cells to ensure that current ghost cells are also
    // present in refined mesh
    p4est_ghost_t *ghost = this->pXest_db_->get_p4est_ghost();
    if (ghost != nullptr) {
      for (size_t jq = 0; jq < ghost->ghosts.elem_count; ++jq) {
        // get ghost quad
        p4est_quadrant_t *ghost_quad =
            p4est_quadrant_array_index(&ghost->ghosts, jq);
        ...
        if (this->get_entities(tdim).find(cell_id, &pos)) {
          LOG_DEBUG(2, "[" << forest->mpirank << "] Keep ghost cell with "
                           << cell_id);
          new_cells.find_insert(cell_id);
    ...
    p4est_refine_ext(forest, 0, -1, pXest_refine_decide_fn,
                     pXest_refine_init_fn, pXest_refine_replace_fn);

    // resort quadrant arrays in forest
    pXest_sort_quad_arrays_in_forest(forest);

    LOG_DEBUG(1, "[" << forest->mpirank
                     << "] Number of cells after refinement: "
                     << new_cells.size());
    LOG_DEBUG(2, "[" << forest->mpirank << "] Cell ids after refinement: "
                     << string_from_range(new_cells.begin(), new_cells.end()));

    // ***************************************************
    // coarsen forest
    ForestCoarsenData coarsen_data;
    pXest_set_forest_user_ptr(forest, &coarsen_data);
    p4est_coarsen_ext(forest, 0, 0, pXest_coarsen_decide_fn,
                      pXest_coarsen_init_fn, pXest_coarsen_replace_fn);

    // resort quadrant arrays in forest
    pXest_sort_quad_arrays_in_forest(forest);

    // remove coarsened cells from list of marked cells
    for (size_t l = 0; l < coarsen_data.coarsened_cells.size(); ++l) {
      int pos;
      ...
      // this->pXest_db_->remove_entity_to_quad_maps(tdim,
      // coarsen_data.coarsened_cells[l], 1);
    }

    // add parent cells of coarsened chidlren
    for (size_t l = 0; l < coarsen_data.parent_cells.size(); ++l) {
      new_cells.find_insert(coarsen_data.parent_cells[l]);
      new_cell_is_local[coarsen_data.parent_cells[l]] = true;
    }

    LOG_DEBUG(1, "[" << forest->mpirank
                     << "] Number of cells after coarsening: "
                     << new_cells.size());
    LOG_DEBUG(2, "[" << forest->mpirank << "] Cell ids after coarsening: "
                     << string_from_range(new_cells.begin(), new_cells.end()));
  } else if (tdim == 3) {
    ...
    if (ghost != nullptr) {
      for (size_t jq = 0; jq < ghost->ghosts.elem_count; ++jq) {
        // get ghost quad
        p8est_quadrant_t *ghost_quad =
            p8est_quadrant_array_index(&ghost->ghosts, jq);
        QuadData *q_ptr = pXest_get_quad_data_ptr(ghost_quad);
        int level = ghost_quad->level;
        const Id cell_id = q_ptr->get_cell_id(level);
    ...
    refine_data.set_children_ids(children_ids_per_parent);
    pXest_set_forest_user_ptr(forest, &refine_data);
    p8est_refine_ext(forest, 0, -1, pXest_refine_decide_fn,
                     pXest_refine_init_fn, pXest_refine_replace_fn);
    ...
    ForestCoarsenData coarsen_data;
    pXest_set_forest_user_ptr(forest, &coarsen_data);
    p8est_coarsen_ext(forest, 0, 0, pXest_coarsen_decide_fn,
                      pXest_coarsen_init_fn, pXest_coarsen_replace_fn);
  ...
  for (size_t l = 0; l < new_cells.size(); ++l) {
    locality.push_back(new_cell_is_local[new_cells.data().at(l)]);
  }

  MeshPtr refined_mesh = MeshPtr(new MeshPXest(
      this->tdim(), this->gdim(), this->pXest_db_, new_cells.data(),
      this->history_index_ + 1, locality, this->period_, this->is_rectangular_,
      &this->mesh_history_));

  LOG_DEBUG(1, " Built adapted intermediate mesh consisting of "
                   << new_cells.size() << " with history index "
                   << this->history_index_ + 1);

  // pass own mesh history to refined mesh, add refined mesh in mesh history of
  // database, add self to mesh history of refined mesh
  ConstMeshPtr self_ptr(this);
  boost::static_pointer_cast< MeshDbView >(refined_mesh)
      ->set_mesh(self_ptr, this->history_index_);

  this->pXest_db_->set_mesh(refined_mesh.get(), this->history_index_ + 1);

  boost::intrusive_ptr< MeshPXest > refined_mesh_pXest =
      boost::static_pointer_cast< MeshPXest >(refined_mesh);

  // ***************************************************
  // balance forest to ensure having at most one hanging node per facet -> add
  // new cells add_cells
  ...
  for (size_t l = 0; l < rm_cells.size(); ++l) {
    int pos;
  ...
  // add new cells
  // CHECK_TODO: are locally owner celles balanced only?
  for (size_t l = 0; l < add_cells.size(); ++l) {
  ...
  MeshPtr balanced_mesh = MeshPtr(new MeshPXest(
      this->tdim(), this->gdim(), this->pXest_db_, new_cells.data(),
      this->history_index_ + 2, locality, this->period_, this->is_rectangular_,
      &refined_mesh_pXest->mesh_history_));
  LOG_DEBUG(1, " Built adapted balanced mesh consisting of "
                   << new_cells.size() << " with history index "
                   << this->history_index_ + 2);

  // pass refined mesh history to balanced mesh, add balanced mesh in mesh
  // history of database, add refined_mesh to mesh history of balanced mesh
  boost::static_pointer_cast< MeshDbView >(balanced_mesh)
      ->set_mesh(refined_mesh, this->history_index_ + 1);

  this->pXest_db_->set_mesh(balanced_mesh.get(), this->history_index_ + 2);

  // ****************************************************
  // ensure that all cells are coarsenable
  if (this->patch_mode_) {
    boost::intrusive_ptr< MeshPXest > balanced_mesh_pXest =
        boost::static_pointer_cast< MeshPXest >(balanced_mesh);
    MeshPtr patched_mesh = balanced_mesh_pXest->make_mesh_coarsenable();

    boost::intrusive_ptr< MeshPXest > patched_mesh_pXest =
        boost::static_pointer_cast< MeshPXest >(patched_mesh);
...
Entity MeshPXest::get_parent_cell(EntityNumber cell_index) const {
  ...
  const int parent_history_index =
      this->pXest_db_->get_last_mesh_history_index(parent_id);

  if (parent_history_index >= 0) {
    // const boost::intrusive_ptr<MeshPXest> parent_mesh =
    // boost::static_pointer_cast<MeshPXest> ( this->pXest_db_->get_mesh (
    // parent_history_index ) );
    MeshPXest *parent_mesh = static_cast< MeshPXest * >(
        this->pXest_db_->get_mesh(parent_history_index));
    const EntityNumber parent_cell_index =
        parent_mesh->get_cell_index(parent_id);
...
std::vector< Id >
MeshPXest::get_children_cell_ids(EntityNumber cell_index) const {
  assert(cell_index >= 0);
  Id cell_id = this->get_id(this->tdim(), cell_index);
  return this->get_descendant_cell_ids(cell_id, 1);
}

std::vector< EntityNumber >
MeshPXest::get_sibling_cell_indices(EntityNumber cell_index) const {
  assert(cell_index >= 0);
  ...
  for (size_t l = 0; l < sibling_ids.size(); ++l) {
...
#ifdef WITH_P4EST
  if (this->tdim() == 2) {
    if (this->pXest_db_->get_p4est_ghost() != nullptr) {
      return this->pXest_db_->get_p4est_ghost()->ghosts.elem_count;
    }
  } else if (this->tdim() == 3) {
    if (this->pXest_db_->get_p8est_ghost() != nullptr) {
...
MeshPtr MeshPXest::extract_boundary_mesh() const {
  ...
  InterfaceList interface_list = InterfaceList::create(this);
  
  // get indices of cells touching the boundary and corresponding facet numbers 
  std::vector< EntityNumber > bdy_cell_indices; 
  std::vector< int > bdy_facet_numbers;
  
  // loop through interfaces
  for (mesh::InterfaceList::const_iterator interface = interface_list.begin(),
       e_interface = interface_list.end(); interface != e_interface; ++interface) 
  {
     // detect boundary interface
     if (interface->num_slaves() == 0)
     {
        bdy_cell_indices.push_back(interface->master_index());
        bdy_facet_numbers.push_back(interface->master_facet_number());
     }
  }

  LOG_DEBUG(2, "num bdy facets: " << bdy_facet_numbers.size() );

  // id:s of boundary facets (used to create boundary mesh)
  std::vector< Id > boundary_facets; 
  
  // indices of boundary facets in current mesh (used to map from bdy mesh to current mesh)
  std::vector< EntityNumber > mesh_facet_indices; 

  // map cell indices and facet numbers to facet ids and indices
  const size_t num_bdy_interfaces = bdy_facet_numbers.size();
  
  for (size_t l=0; l<num_bdy_interfaces; ++l)
  {
    // get entity object of current cell
    Entity bdy_cell (this, mesh_tdim, bdy_cell_indices[l]);
    
    // loop through incident facets
    int cur_facet_nr = 0;
    bool found_facet = false;
    LOG_DEBUG(2, "Find facet id and index for facet number " << bdy_facet_numbers[l] << 
                 " on cell " << bdy_cell_indices[l] << 
                 " with " << this->num_incident_entities(bdy_cell, mesh_tdim-1) << " incident facets");
    
    for (IncidentEntityIterator i_facet = bdy_cell.begin_incident(mesh_tdim-1), 
         e_facet = bdy_cell.end_incident(mesh_tdim-1);
         i_facet != e_facet; ++i_facet)
    {
      if (cur_facet_nr == bdy_facet_numbers[l])
      {
        boundary_facets.push_back(i_facet->id());
        mesh_facet_indices.push_back(i_facet->index());
        found_facet = true;
        break;
      }
      cur_facet_nr++;
...
#ifndef NDEBUG
    int face_number;
    const bool check = this->find_entity(mesh_tdim - 1, boundary_facets[l], &face_number);

    LOG_DEBUG(2, " bdy cell index " << bdy_cell_indices[l] << std::endl << 
                 " bdy facet nr " << bdy_facet_numbers[l] << std::endl << 
                 " bdy facet id " << boundary_facets[l] << std::endl << 
                 " bdy facet index " << mesh_facet_indices[l] << std::endl << 
                 " checked facet index " << face_number);
    assert(check);
    assert(face_number == mesh_facet_indices[l]);
#endif
  }

  if (!boundary_facets.empty()) 
  {
    // compute permutation that sorts boundary_facets (as will be done in the
    // constructor)
    ...
    MeshPtr bdy_mesh(new MeshDbView(mesh_tdim - 1, gdim(), this->db_, boundary_facets));
...
void MeshPXest::save(std::string filename, const MPI_Comm &comm) const {
  ...
  for (size_t i = 0; i < bool_to_int.size(); ++i) {
    bool_to_int[i] = static_cast< int >(cell_is_local_[i]);
  }
  write_array_parallel< int >(group_ptr, "cell_is_local", bool_to_int, comm);

  write_array_parallel(group_ptr, "connection_mode", &connection_mode_, 1,
                       comm);

  int temp_patch_mode = static_cast< int >(patch_mode_);
...
void MeshPXest::load(std::string filename, const MPI_Comm &comm) {
  ...
  for (size_t i = 0; i < bool_to_int.size(); ++i) {
    cell_is_local_[i] = static_cast< bool >(bool_to_int[i]);
  }
  ...
  assert(dummy == 1);
  patch_mode_ = static_cast< bool >(*buffer);
  free(buffer);
#endif
}

void MeshPXest::copy_from(ConstMeshPtr mesh) {
  // copy member variables defined in MeshDbView class
  boost::intrusive_ptr< const MeshPXest > mesh_pXest =
      boost::static_pointer_cast< const MeshPXest >(mesh);
  ...
  // copy pointers to old meshes
  this->mesh_history_ = mesh_pXest->mesh_history_;
  ...
  if (attr != nullptr) {
    ...
    for (std::vector< int >::iterator it_att = int_att.begin();
         it_att < int_att.end(); ++it_att, ++i) {
      *it_att = int_attr->get_int_value(i);
    }

    AttributePtr new_attribute = AttributePtr(new IntAttribute(int_att));
    this->add_attribute(std::string("__sub_cell_number__"), tdim,
                        new_attribute);
...
void MeshPXest::deep_copy_from(ConstMeshPtr mesh) {
  // this loop has to be called to avoid later errors due to missing entities in
  // input mesh. num_entities creates all necessary entities in input mesh
  const int tdim = mesh->tdim();
  for (int dim = 0; dim < tdim + 1; ++dim) {
    mesh->num_entities(dim);
  }

  // copy database
  MeshPXestDatabasePtr tmp_db(
      new MeshPXestDatabase(this->tdim(), this->gdim()));

  boost::intrusive_ptr< const MeshPXest > mesh_pXest =
      boost::static_pointer_cast< const MeshPXest >(mesh);
  tmp_db->copy_from(mesh_pXest->get_db());

  // standard copy of mesh
  this->copy_from(mesh);

  // copy meshes in history
  std::map< int, ConstMeshPtr > tmp_history;

  // loop over all meshes
  for (int h = 0; h < this->history_index_; ++h) {
    // get current mesh to copy from history of input mesh
    ConstMeshPtr cur_mesh =
        boost::static_pointer_cast< const MeshDbView >(mesh)->get_mesh(h);

    assert(cur_mesh != nullptr);

    // create new mesh and copy from input mesh
    MeshPtr tmp_mesh(new MeshPXest(this->tdim(), this->gdim()));
    tmp_mesh->copy_from(cur_mesh);

    // update mesh history in copied to -> contains now all previuosly created
    // meshes
    boost::static_pointer_cast< MeshDbView >(tmp_mesh)->set_mesh_history(
        tmp_history);
    tmp_history[h] = tmp_mesh;

    // update database pointer in copied mesh
    boost::intrusive_ptr< MeshPXest > tmp_mesh_pXest =
        boost::static_pointer_cast< MeshPXest >(tmp_mesh);
    tmp_mesh_pXest->set_db(tmp_db);

    // update mesh pointer in database
    tmp_db->set_mesh(tmp_mesh.get(), h);
  }

  // do the same in this mesh
  this->mesh_history_ = tmp_history;
  this->set_db(tmp_db);
  tmp_db->set_mesh(this, this->history_index_);
}
...
bool MeshPXest::is_uniformly_coarsenable() const {
  ...
  LOG_DEBUG(
      1, "Number of non coarsenable cells: " << non_coarsenable_cells.size());
  return non_coarsenable_cells.empty();
}
...
void MeshPXest::set_connection_mode(int mode) {
  ...
  if (this->patch_mode_) {
    ...
    if (mode == 0) {
      LOG_DEBUG(0, "Caution: You set patch_mode = true. This choice requires "
                   "connection_mode = 1 (2D) or 2 (3D)! ");
    }
  }
}
...
void MeshPXest::create_interface_list(InterfaceList *list) const {
  ...
  // iterate over forest and call function "p4est_interface_list_fn" on each
  // facet
  // TODO facets zwischen ghost cells nötig?
  if (this->tdim() == 2) {
    p4est_iterate(this->pXest_db_->get_p4est_forest(),
                  this->pXest_db_->get_p4est_ghost(), list, nullptr,
                  pXest_interface_list_fn, nullptr);
  }
  if (this->tdim() == 3) {
    p8est_iterate(this->pXest_db_->get_p8est_forest(),
                  this->pXest_db_->get_p8est_ghost(), list, nullptr,
                  pXest_interface_list_fn, nullptr, nullptr);
  }
#endif
}

// ---------------------------------------------------------------------
// BUG_TODO: check whether works fine in case of recursive refinement of
// balanced cell

void MeshPXest::balance(std::vector< Id > &add_cells,
                        std::vector< Id > &rm_cells,
                        int connection_mode) const {
  ...
  // call p4est_balance routine.  Refined cell indices are stored in
  // balance_data -> ancestor_cells
  ...
  if (tdim == 2) {
    ...
    if (connection_mode >= 1) {
      p4est_balance_ext(forest4, P4EST_CONNECT_FULL, pXest_balance_init_fn,
                        pXest_balance_replace_fn);
    } else {
      p4est_balance_ext(forest4, P4EST_CONNECT_FACE, pXest_balance_init_fn,
                        pXest_balance_replace_fn);
    }
    ...
    if (connection_mode == 2) {
      p8est_balance_ext(forest8, P8EST_CONNECT_FULL, pXest_balance_init_fn,
                        pXest_balance_replace_fn);
    } else if (connection_mode == 1) {
      p8est_balance_ext(forest8, P8EST_CONNECT_EDGE, pXest_balance_init_fn,
                        pXest_balance_replace_fn);
    } else {
      p8est_balance_ext(forest8, P8EST_CONNECT_FACE, pXest_balance_init_fn,
                        pXest_balance_replace_fn);
    }
  ...
  LOG_DEBUG(1, " Number of replaced cells "
                   << balance_data.replaced_cell_indices.size());
  ...
  for (int l = 0; l < num_replaced_cells; ++l) {
    ...
    LOG_DEBUG(2, " Balance (i.e. refine) ancestor index "
                     << ancestor_index << " with cell id " << ancestor_id);
    rm_cells.push_back(ancestor_id);

    // get quadrant of replaced ancestor cell ******************
    QuadCoord ancestor_coord =
        this->pXest_db_->get_entity_to_quad_coord(tdim, ancestor_id, 1);
    ...
    if (tdim == 2) {
      ...
      LOG_DEBUG(2, " Ancestor is on level "
                       << static_cast< int >(ancestor_quad4.level)
                       << " and on tree " << ancestor_quad4.p.which_tree);
      ...
      tree_quads4.insert(tree_quads4.end(),
                         balance_data.new_quads4[ancestor_index].begin(),
                         balance_data.new_quads4[ancestor_index].end());
      ...
      LOG_DEBUG(2, " Ancestor is on level "
                       << static_cast< int >(ancestor_quad8.level)
                       << " and on tree " << ancestor_quad8.p.which_tree);

      // get descendant quadrants of current ancestor ************
      num_desc = balance_data.new_quads8[ancestor_index].size();
      tree_quads8[0] = ancestor_quad8;
      tree_quads8.insert(tree_quads8.end(),
                         balance_data.new_quads8[ancestor_index].begin(),
                         balance_data.new_quads8[ancestor_index].end());
    ...
    if (DEBUG_LEVEL >= 2) {
      for (int jn = 0; jn < num_cells; ++jn) {
        ...
        LOG_DEBUG(2, "Tree node " << jn << " has parent cell "
                                  << cur_cell.parent() << " and "
                                  << cur_cell.num_children()
                                  << " children and sub_cell_number "
                                  << cur_cell.sub_cell_number());
        ...
        if (cur_cell.parent() >= 0) {
          ...
          if (tdim == 2) {
            ...
            p4est_quadrant_t parent_quad =
                tree_quads4[parent_cell.sub_cell_number()];
            LOG_DEBUG(2, " This should be true: " << p4est_quadrant_is_parent(
                             &parent_quad, &cur_quad));
          } else if (tdim == 3) {
            p8est_quadrant_t cur_quad = tree_quads8[cur_cell.sub_cell_number()];
            p8est_quadrant_t parent_quad =
                tree_quads8[parent_cell.sub_cell_number()];
            LOG_DEBUG(2, " This should be true: " << p8est_quadrant_is_parent(
                             &parent_quad, &cur_quad));
          }
        }
      }
    ...
    desc_ids = this->compute_refined_cells_recursive(
        ancestor_cell, &ref_tree, local_sub_cell_numbers, tree_node_numbers,
        nullptr, false);
    assert(local_sub_cell_numbers.size() == num_desc);

    LOG_DEBUG(2, "Refining cell " << ancestor_index << " yields "
                                  << desc_ids.size() << " new cells with ids ");
    if (DEBUG_LEVEL >= 2) {
    ...
    // and select those newly created cells for the new mesh that correspond to
    // a leaf quad

    // Loop over all cells in refinementTree except of root
    for (size_t j = 0; j < tree_node_numbers.size(); ++j) {
      ...
      assert (cur_level >= 0);
      LOG_DEBUG(2, "Current quad with quadrant nr "
                       << jq << " has level " << cur_level
                       << ", tree_node_nr: " << jn << ", cell id: " << cur_id
                       << ", is leaf: " << !cur_cell.is_refined());

      if (!cur_cell.is_refined()) { // cur_quad is leaf

        // put corresponding cell_id into add_cells
        add_cells.push_back(cur_id);

        // get QuadData pointer of current desc quadrant
        QuadData *q_ptr = nullptr;

        if (tdim == 2) {
          treeId tree_id = ancestor_quad4.p.which_tree;
          p4est_tree_t *tree = pXest_get_tree_in_forest(forest4, tree_id);
          LOG_DEBUG(2, "Current leaf: Tree: "
                           << tree_id << " level "
                           << static_cast< int >(cur_quad4.level) << " x "
                           << cur_quad4.x << " y " << cur_quad4.y);
        ...
        // get associated cell ids of all quadrants between ancestor_quad and
        // cur_quad and store them in QuadData of leaf quadrant
        ...
        while (!reached_root) {
          ...
          LOG_DEBUG(2, " Traverse: it_cell_id: " << it_id << " level "
                                                 << it_level);
          ...
          if (it_cell->is_root()) {
            ...
            LOG_DEBUG(2, " Traverse: parent nr: " << it_nr << " index "
                                                  << it_index);
...
MeshPtr MeshPXest::refine_uniform_seq() const {
  ...
  for (int index = 0; index < num_cells; ++index) {
    ...
    const Entity current_cell = this->get_entity(tdim, index);
    RefinementTree ref_tree(
        tdim, current_cell.cell_type(), num_ref_steps,
        static_cast< int >(std::pow(static_cast< double >(2), tdim)));

    std::vector< int > local_sub_cell_numbers;
    std::vector< int > tree_node_numbers;
    std::vector< Id > desc_ids = this->compute_refined_cells_recursive(
        current_cell, &ref_tree, local_sub_cell_numbers, tree_node_numbers,
        nullptr, true);
    ...
    for (size_t j = 0; j < tree_node_numbers.size(); ++j) {
      ...
      if (!cur_cell.is_refined()) { // cell is leaf
  ...
  MeshPtr refined_mesh =
      MeshPtr(new MeshPXest(tdim, gdim, this->pXest_db_, new_cells.data(), 0,
                            locality, this->period_, this->is_rectangular_));
  boost::intrusive_ptr< MeshPXest > refined_mesh_p4est =
      boost::static_pointer_cast< MeshPXest >(refined_mesh);
  ...
  if (tdim == 2) {
    assert(num_cells * static_cast< int >(
                           std::pow(static_cast< double >(4), num_ref_steps)) ==
           refined_mesh_p4est->num_entities(tdim));
  }
  if (tdim == 3) {
    assert(num_cells * static_cast< int >(
                           std::pow(static_cast< double >(8), num_ref_steps)) ==
           refined_mesh_p4est->num_entities(tdim));
  }
  ...
  for (EntityIterator it = refined_mesh_p4est->begin(tdim),
                      end_it = refined_mesh_p4est->end(tdim);
       it != end_it; ++it) {
    // get vertex ids of current cell
    for (VertexIdIterator it_v = it->begin_vertex_ids();
         it_v != it->end_vertex_ids(); ++it_v) {
  ...
  for (int jv = 0; jv < num_vertices; ++jv) {
    ...
    // refined_mesh_p4est->get_db()->get_coordinates(v_id);
    std::vector< Coordinate > coords =
        refined_mesh_p4est->get_db()->get_coordinates(jv);
  ...
  for (EntityIterator it = refined_mesh_p4est->begin(tdim),
    ...
    p4est_tree_to_vertices.insert(p4est_tree_to_vertices.end(), v.begin(),
                                  v.end());

    // store entitiy(tdim)-to-quad map
    // [tree_id, ref_level, x, y, z, (as topological coordinates inside of
    // cell), localId, topological dim, id in coarse mesh]

    QuadCoord coord(it->index(), 0, 0, 0, 0, 0, tdim, it->id());
    refined_mesh_p4est->get_db()->add_entity_to_quad_coord(tdim, it->id(),
                                                           coord, 0);
    LOG_DEBUG(2, "added coarse entity_to_quad map for cell id "
                     << it->id() << " with corresponding tree id "
                     << it->index());

    assert(it->index() == counter);
    counter++;
  }
  refined_mesh_p4est->get_db()->set_conn_data(
      num_vertices, num_cells, p4est_vertices, p4est_tree_to_vertices);
...
MeshPtr MeshPXest::add_ghost_cells(MPI_Comm comm, int layer_width) const {
  ...
  LOG_DEBUG(1, "Compute ghost cells with layer width " << layer_width);
  int rank = -1;
  ...
  // exchange data in ghost layers and put it into database, sub_domains and
  // remote_indices
  ...
  for (size_t l = 0; l < new_cells[this->history_index_].size(); ++l) {
  ...
  for (size_t l = 0; l < old_ghosts.size(); ++l) {
  ...
  for (size_t l = 0; l < fine_cells.size(); ++l) {
  ...
  LOG_DEBUG(3, "[" << rank << "] Mesh " << this->history_index_
                   << " : cell ids after adding ghost cells: "
                   << string_from_range(fine_cells.begin(), fine_cells.end()));

  MeshPtr mesh_with_ghost = MeshPtr(
      new MeshPXest(this->tdim(), this->gdim(), this->pXest_db_, fine_cells,
                    this->history_index_, cell_is_local, this->period_,
                    this->is_rectangular_, &this->mesh_history_));

  mesh_with_ghost->set_ghost_layer_width(layer_width);

  LOG_DEBUG(1, "[" << rank << "] Number of cells after adding ghost layer: "
                   << mesh_with_ghost->num_entities(tdim));

  // boost::intrusive_ptr<MeshPXest> mesh_pXest =
  // boost::static_pointer_cast<MeshPXest> ( mesh_with_ghost );
  // mesh_pXest->set_cell_flags(cell_is_local);

  // Update pointer to this mesh in database
  this->pXest_db_->set_mesh(mesh_with_ghost.get(), this->history_index_);

  assert(this->pXest_db_->get_mesh(this->history_index_) == mesh_with_ghost);
  assert(this->pXest_db_->get_mesh(this->history_index_)
             ->num_entities(this->tdim()) == fine_cells.size());
  ...
  for (int hi = 0; hi < this->history_index_; ++hi) {
    if (!new_cells[hi].empty()) {
      // boost::intrusive_ptr<MeshPXest> cur_mesh =
      // boost::static_pointer_cast<MeshPXest> ( this->pXest_db_->get_mesh ( hi
      // )
      // );
      MeshPXest *cur_mesh =
          static_cast< MeshPXest * >(this->pXest_db_->get_mesh(hi));
      LOG_DEBUG(2, " Mesh " << hi << " gets possibly " << new_cells[hi].size()
                            << " new ghost cells ");

      cur_mesh->add_cells_and_rebuild(new_cells[hi]);
      for (size_t l = 0; l < new_cells[hi].size(); ++l) {
  ...
  boost::static_pointer_cast< MeshPXest >(mesh_with_ghost)
      ->init_ghost_indices();
  boost::static_pointer_cast< MeshPXest >(mesh_with_ghost)
      ->exchange_ghost_indices();
  boost::static_pointer_cast< MeshPXest >(mesh_with_ghost)
      ->compute_subdomains_and_remote_indices(sub_domains, remote_indices);
...
SortedArray< Id > MeshPXest::get_ghost_ids() const {
  ...
  for (size_t jq = 0; jq < num_ghosts; ++jq) {
    // get ghost quad data pointer
    QuadData *q_ptr = nullptr;
    int level = -1;
    if (tdim == 2) {
      p4est_quadrant_t *ghost_quad =
          p4est_quadrant_array_index(&ghost4->ghosts, jq);
      q_ptr = pXest_get_quad_data_ptr(ghost_quad);
      level = ghost_quad->level;
    } else if (tdim == 3) {
      p8est_quadrant_t *ghost_quad =
          p8est_quadrant_array_index(&ghost8->ghosts, jq);
      q_ptr = pXest_get_quad_data_ptr(ghost_quad);
      level = ghost_quad->level;
    }
...
void MeshPXest::init_ghost_layer(int layer_width) const {
  ...
  if (this->tdim() == 2) {
    ...
    p4est_ghost_t *ghost = p4est_ghost_new(this->pXest_db_->get_p4est_forest(),
                                           P4EST_CONNECT_FULL);
  ...
  if (this->tdim() == 3) {
    // create new ghost layer
    p8est_ghost_t *ghost = p8est_ghost_new(this->pXest_db_->get_p8est_forest(),
                                           P8EST_CONNECT_FULL);
...
void MeshPXest::init_ghost_data() const {
  ...
  // get mirror cells -> local cells that are in ghost layer of any other
  // process
  if (tdim == 2) {
    p4est_t *forest = this->pXest_db_->get_p4est_forest();
    p4est_ghost_t *ghost = this->pXest_db_->get_p4est_ghost();
    ...
    for (size_t jq = 0; jq < num_mirrors; ++jq) {
      // get mirror quad
      p4est_quadrant_t *mirror_quad =
          p4est_quadrant_array_index(&ghost->mirrors, jq);
      treeId jt = mirror_quad->p.piggy3.which_tree;

      // extract quad from corresponding local tree
      p4est_tree_t *tree = pXest_get_tree_in_forest(forest, jt);
      p4est_locidx_t which_quad =
          mirror_quad->p.piggy3.local_num - tree->quadrants_offset;
      p4est_quadrant_t *mirror_quad_with_data =
      ...
      if (index >= 0) {
        ...
        for (int l = level; l >= 0; --l) {
          ...
          int history_index =
              this->pXest_db_->get_last_mesh_history_index(cell_id);
          mirror_pack->mesh_history[l] = history_index;

          LOG_DEBUG(3, "Pack cell id " << cell_id << " on level " << l
                                       << " with history index "
                                       << history_index);
          // Pack cell entity
          ...
          this->pXest_db_->build(tdim - 1, ids, incident_facets,
                                 cell_d_connections);

          for (size_t jf = 0; jf < incident_facets.size(); ++jf) {
            std::vector< Id > facet_ids(1);
            facet_ids[0] = incident_facets[jf];
            this->pXest_db_->create_entity_package(
                tdim - 1, facet_ids, &facet_packs[facet_pack_offset]);
            ++facet_pack_offset;
          }
        }
    ...
    for (size_t jq = 0; jq < num_mirrors; ++jq) {
      // get mirror quad
      p8est_quadrant_t *mirror_quad =
          p8est_quadrant_array_index(&ghost->mirrors, jq);
      treeId jt = mirror_quad->p.piggy3.which_tree;
      ...
      p4est_locidx_t which_quad =
          mirror_quad->p.piggy3.local_num - tree->quadrants_offset;
      p8est_quadrant_t *mirror_quad_with_data =
          p8est_quadrant_array_index(&tree->quadrants, which_quad);
      QuadData *q_ptr = pXest_get_quad_data_ptr(mirror_quad_with_data);
...
void MeshPXest::exchange_ghost_data(
    std::vector< SortedArray< Id > > &new_cells) const {
  LOG_DEBUG(1, "Exchange ghost cells ");
  MeshPXestBuilder builder(this->tdim(), this->gdim(), this->get_db(),
                           this->get_entities(this->tdim()));
#ifdef WITH_P4EST
  int tdim = this->tdim();
  size_t num_ghosts;

  // exchange ghost data with p4est and retrieve entity_packages from ghost
  // quadrants
  ...
  if (tdim == 2) {
    ...
    GhostCommData22 *ghost_data_22 = new GhostCommData22[num_ghosts];

    p4est_ghost_exchange_custom(forest, ghost, sizeof(GhostCommData22),
                                (void **)(&this->mirror_data_22_[0]),
                                static_cast< void * >(ghost_data_22));
    num_ghosts = ghost->ghosts.elem_count;
    ...
    for (size_t jq = 0; jq < ghost->ghosts.elem_count; ++jq) {
      ...
      if (remote_index >= 0) {
        ...
        size_t num_cells = cell_packs.size();
        int level = ghost_quad->level;
        ...
        for (size_t l = 0; l <= level; ++l) {
          std::vector< Id > new_cell_id =
              unpack_entities(cell_packs[l], builder);
          ...
          LOG_DEBUG(3, "UnPack cell id " << new_cell_id[0] << " on level " << l
                                         << " with history index "
                                         << history_index[0]);
        }

        // put facets into database
        for (size_t l = 0; l < facet_packs.size(); ++l) {
        ...
        for (int l = level; l >= 0; --l) {
          ...
          this->pXest_db_->add_entity_to_quad_coord(tdim, ghost_ids[l], coord,
                                                    1);
    ...
    // free ghost_data
    delete[] ghost_data_22;
    for (size_t l = 0; l < this->mirror_data_22_.size(); ++l) {
      delete this->mirror_data_22_[l];
    }
  } else if (tdim == 3) {
    p8est_ghost_t *ghost = this->pXest_db_->get_p8est_ghost();
    p8est_t *forest = this->pXest_db_->get_p8est_forest();
    ...
    // init some data structures
    num_ghosts = ghost->ghosts.elem_count;
    GhostCommData33 *ghost_data_33 = new GhostCommData33[num_ghosts];

    p8est_ghost_exchange_custom(forest, ghost, sizeof(GhostCommData33),
                                (void **)(&this->mirror_data_33_[0]),
                                static_cast< void * >(ghost_data_33));
    num_ghosts = ghost->ghosts.elem_count;
    ...
    // free ghost_data
    delete[] ghost_data_33;
    for (size_t l = 0; l < this->mirror_data_33_.size(); ++l) {
      delete this->mirror_data_33_[l];
    }
  }
#endif
}
...
void MeshPXest::init_ghost_indices() const {
  ...
  // process
  if (tdim == 2) {
    assert(ghost4 != nullptr);
    num_mirrors = ghost4->mirrors.elem_count;
  } else if (tdim == 3) {
    assert(ghost8 != nullptr);
  ...
  for (size_t jq = 0; jq < num_mirrors; ++jq) {
    ...
    if (tdim == 2) {
      p4est_quadrant_t *mirror_quad =
          p4est_quadrant_array_index(&ghost4->mirrors, jq);
      treeId jt = mirror_quad->p.piggy3.which_tree;
      ...
      p8est_quadrant_t *mirror_quad =
          p8est_quadrant_array_index(&ghost8->mirrors, jq);
      treeId jt = mirror_quad->p.piggy3.which_tree;
...
void MeshPXest::exchange_ghost_indices() const {
  ...
  p8est_t *forest8 = this->pXest_db_->get_p8est_forest();
  size_t num_ghosts = 0;
  GhostCommData *ghost_data = nullptr;
  ...
  if (tdim == 2) {
    ...
    num_ghosts = ghost4->ghosts.elem_count;
    ghost_data = new GhostCommData[num_ghosts];
    p4est_ghost_exchange_custom(forest4, ghost4, sizeof(GhostCommData),
                                (void **)(&this->mirror_data_[0]),
                                static_cast< void * >(ghost_data));
  }
  if (tdim == 3) {
    assert(ghost8 != nullptr);
    ...
    num_ghosts = ghost8->ghosts.elem_count;
    ghost_data = new GhostCommData[num_ghosts];
    p8est_ghost_exchange_custom(forest8, ghost8, sizeof(GhostCommData),
                                (void **)(&this->mirror_data_[0]),
                                static_cast< void * >(ghost_data));
  }
  ...
  // free ghost_data
  delete[] ghost_data;
  for (size_t l = 0; l < this->mirror_data_.size(); ++l) {
    delete this->mirror_data_[l];
  }
#endif
}
...
void MeshPXest::compute_subdomains_and_remote_indices(
    std::vector< SubDomainId > &sub_domains,
    std::vector< EntityNumber > &remote_indices) const {
#ifdef WITH_P4EST
  int tdim = this->tdim();
  ...
  for (size_t j = 0; j < q_ptrs.size(); ++j) {
    ...
    LOG_DEBUG(3, "Index: " << index << " remote index: " << remote_index
                           << " subdomain: " << owner_rank << " my rank "
                           << my_rank);
...
void MeshPXest::find_coarsenable_cells(
    SortedArray< EntityNumber > &coarsenable_cells) const {
  ...
  if (this->tdim() == 2) {
    ...
    p4est_coarsen_ext(forest, 0, 0, pXest_coarsen_find_fn,
                      pXest_coarsen_init_fn, pXest_coarsen_replace_fn);
  } else if (this->tdim() == 3) {
    p8est_t *forest = this->pXest_db_->get_p8est_forest();
    pXest_set_forest_user_ptr(forest, &patch_data);
    p8est_coarsen_ext(forest, 0, 0, pXest_coarsen_find_fn,
                      pXest_coarsen_init_fn, pXest_coarsen_replace_fn);
  }

  // PERF_TODO: use SortedArray copy operator
  for (size_t l = 0; l < patch_data.coarsenable_cells.size(); ++l) {
...
MeshPtr MeshPXest::make_mesh_coarsenable() {
  ...
  while (!finished) {
    ...
    LOG_DEBUG(1, "Iteration: "
                     << iter << " total number of cells: " << num_cells
                     << " coarsenable cells: " << coarsenable_cells.size()
                     << " non coarsenable local cells: "
                     << non_coarsenable_cells.size());

    int local_num_non_coarsenable_cells = non_coarsenable_cells.size();
    int global_num_non_coarsenable_cells = 0;
    MPI_Allreduce(&local_num_non_coarsenable_cells,
                  &global_num_non_coarsenable_cells, 1, MPI_INT, MPI_SUM,
                  *mpi_comm);
    ...
    for (size_t l = 0; l < non_coarsenable_cells.size(); ++l) {
...
Id MeshPXest::get_ancestor_cell_id(Id cell_id, int level_diff) const {
  ...
  LOG_DEBUG(3, "id " << cell_id << " level " << level << " x " << x << " y "
                     << y << " z " << z << " tree " << tree_id);
  if (level < 1) {
    return -1;
  }
  if (tdim == 2) {
    ...
    if (tree_id >= first_tree && tree_id <= last_tree) { // tree is local
      LOG_DEBUG(3, "Search local trees ");

      // get tree
      p4est_tree_t *tree =
          pXest_get_tree_in_forest(pXest_db_->get_p4est_forest(), tree_id);

      q_ptr = pXest_get_quad_data_ptr(tree, &tmp_quad);
      if (q_ptr != nullptr) { // tmp_quad contains required data
        if (q_ptr->get_cell_id(level) == cell_id) {
          LOG_DEBUG(3, "Requested cells corresponds to local leaf ");
          quad_is_local = true;
        }
      } else { // either tmp_quad is in the ghost layer, or tmp_quad is not a
               // leaf, i.e. it contains no data
        // try to find descendant of tmp_quad which is a local leaf

        // Loop over all quads in current tree
        size_t num_quads = tree->quadrants.elem_count;
        for (size_t jq = 0; jq < num_quads; ++jq) {
          ...
          if (pXest_quad_is_ancestor(&tmp_quad, cur_quad)) {
            LOG_DEBUG(3,
                      "Requested cells corresponds to ancestor of local leaf ");
    ...
    if (!quad_is_local) { // search ghost layer
        ...
        for (size_t jq = 0; jq < ghost->ghosts.elem_count; ++jq) {
          ...
          if (q_ptr->get_cell_id(level) == cell_id) {
            ...
            if (DEBUG_LEVEL > 0)
            {
                ...
                {
                    LOG_DEBUG(1, "x : " << ghost_quad->x << " <-> " <<
            tmp_quad.x ); LOG_DEBUG(1, "y : " << ghost_quad->y << " <-> " <<
            tmp_quad.y ); LOG_DEBUG(1, "level : " << (int)ghost_quad->level << "
            <-> " << (int)tmp_quad.level ); q_ptr->print(); coord.print();
                }
            }
             * */
            break;
    ...
    if (tree_id >= first_tree && tree_id <= last_tree) { // tree is local
      ...
      // get tree
      p8est_tree_t *tree =
          pXest_get_tree_in_forest(pXest_db_->get_p8est_forest(), tree_id);
          ...
          if (q_ptr->get_cell_id(level) == cell_id) {
            ...
            if (DEBUG_LEVEL > 0)
            {
                if (!p8est_quadrant_is_equal(ghost_quad, &tmp_quad))
                {
                    LOG_DEBUG(1, "x : " << ghost_quad->x << " <-> " <<
            tmp_quad.x ); LOG_DEBUG(1, "y : " << ghost_quad->y << " <-> " <<
            tmp_quad.y ); LOG_DEBUG(1, "z : " << ghost_quad->z << " <-> " <<
            tmp_quad.z ); LOG_DEBUG(1, "level : " << (int)ghost_quad->level << "
            <-> " << (int)tmp_quad.level ); q_ptr->print(); coord.print();
                }
            }
             * */
...
std::vector< Id > MeshPXest::get_descendant_cell_ids(Id cell_id,
  ...
  if (tdim == 2) {
    ...
    if (tree_id >= first_tree && tree_id <= last_tree) { // tree is local
      ...
      if (pos >=
          0) { // tmp_quad contains data -> is a leaf, i.e. has no children
        LOG_DEBUG(3, " Cell is a leaf -> has no descendants, position " << pos);
        return children_ids;
      }

      // search descendants of tmp_quad which are leafs
      std::vector< int64_t > leaf_pos_in_tree =
          pXest_get_leaf_desc_in_tree_array(tree, &tmp_quad);
      LOG_DEBUG(3,
                " Search " << leaf_pos_in_tree.size() << " leaf descendants");
      for (size_t l = 0; l < leaf_pos_in_tree.size(); ++l) {
        p4est_quadrant_t *cur_quad =
            pXest_get_local_quad_in_tree(tree, leaf_pos_in_tree[l]);
        QuadData *q_ptr = pXest_get_quad_data_ptr(cur_quad);
        assert(q_ptr != nullptr);
    ...
    if (ghost != nullptr) {
      ...
      int64_t pos =
          pXest_get_quad_pos_in_ghost_array(ghost, &tmp_quad, tree_id);

      std::vector< int64_t > leaf_pos_in_ghost =
          pXest_get_leaf_desc_in_ghost_array(ghost, &tmp_quad, tree_id);
      LOG_DEBUG(3,
                " Search " << leaf_pos_in_ghost.size() << " ghost descendants");
      for (size_t l = 0; l < leaf_pos_in_ghost.size(); ++l) {
        p4est_quadrant_t *cur_quad =
            pXest_get_local_quad_in_ghost(ghost, leaf_pos_in_ghost[l]);
        QuadData *q_ptr = pXest_get_quad_data_ptr(cur_quad);
        assert(q_ptr != nullptr);
        Id desc_id = q_ptr->get_cell_id(level + level_diff);
        if (desc_id >= 0) {
...
void MeshPXest::update_cell_index_in_quads(bool update_ghost) {
  ...
  if (this->tdim() == 2) {
    ...
    for (treeId jt = first_treeId; jt <= last_treeId; ++jt) {
      ...
      for (size_t jq = 0; jq < num_quads; ++jq) {
        p4est_quadrant_t *quad =
            p4est_quadrant_array_index(&tree->quadrants, jq);

        QuadData *q_ptr = pXest_get_quad_data_ptr(quad);
        q_ptr->set_cell_index(
            this->get_cell_index(q_ptr->get_cell_id(quad->level)));
        q_ptr->set_remote_cell_index(
            this->get_cell_index(q_ptr->get_cell_id(quad->level)));
      }
    }
    if (update_ghost && ghost != nullptr) {
      ...
      for (size_t jq = 0; jq < ghost->ghosts.elem_count; ++jq) {
        ...
        QuadData *q_ptr = pXest_get_quad_data_ptr(ghost_quad);
        q_ptr->set_cell_index(
            this->get_cell_index(q_ptr->get_cell_id(ghost_quad->level)));
      }
    }
  } else if (this->tdim() == 3) {
    p8est_t *forest = this->pXest_db_->get_p8est_forest();
    ...
    for (treeId jt = first_treeId; jt <= last_treeId; ++jt) {
      ...
      for (size_t jq = 0; jq < num_quads; ++jq) {
        p8est_quadrant_t *quad =
            p8est_quadrant_array_index(&tree->quadrants, jq);
...
void MeshPXest::update_cell_id_to_index_map() {
  this->cell_id_to_index_.clear();
  for (EntityNumber index = 0; index != this->num_entities(this->tdim());
       ++index) {
    this->cell_id_to_index_.insert(std::pair< Id, EntityNumber >(
        this->get_id(this->tdim(), index), index));
  }
}
...
int MeshPXest::get_sub_cell_number(Id cell_id) const {
  ...
  LOG_DEBUG(3, "id " << cell_id << " level " << level << " x " << x << " y "
                     << y << " z " << z << " tree " << tree_id
                     << "Quad sibling number: " << quad_sibling_number
                     << " sub cell number: " << sub_cell_number);
...
void MeshPXest::update_sub_cell_numbers() {
  ...
  this->sub_cell_number_attr_ =
      AttributePtr(new IntAttribute(sub_cell_numbers));
  this->add_attribute(std::string("__sub_cell_number__"), this->tdim(),
                      sub_cell_number_attr_);
}
...
void MeshPXest::add_cells_and_rebuild(SortedArray< Id > &new_cells) {
  ...
  LOG_DEBUG(3,
            "Mesh " << this->history_index_ << ": old cells "
                    << string_from_range(cells.begin(), cells.end())
                    << ", added cells: "
                    << string_from_range(new_cells.begin(), new_cells.end()));
...
void MeshPXest::rebuild(std::vector< Id > &cells) {
  ...
  LOG_DEBUG(2, "Rebuild mesh "
                   << this->history_index_ << " with cells "
                   << string_from_range(cells.begin(), cells.end()));
  int tdim = this->tdim();
...
std::vector< Id > MeshPXest::compute_refined_cells_recursive(
    const Entity &cell, RefinementTree *tree,
    std::vector< int > &sub_cell_numbers, std::vector< int > &tree_node_numbers,
    MeshPXestBuilder *builder, bool leaf_only) const {
  ...
  ::hiflow::mesh::compute_refined_cells_recursive(
      cell, *tree, refined_cell_coordinates, refined_cell_sizes,
      sub_cell_numbers, tree_node_numbers, leaf_only, this->rec_ref_geom_fun_);

  LOG_DEBUG(2, " Parent cell id " << cell.id());
  LOG_DEBUG(2, " Num descendants " << refined_cell_sizes.size()
                                   << " coord size "
                                   << refined_cell_coordinates.size());
  LOG_DEBUG(2, " Tree node numbers " << string_from_range(
                   tree_node_numbers.begin(), tree_node_numbers.end()));
  LOG_DEBUG(3, " Refined cell coords size "
                   << string_from_range(refined_cell_coordinates.begin(),
                                        refined_cell_coordinates.end()));
  ...
  for (int v = 0; v < num_refined_vertices; ++v) {
    if (builder == nullptr) {
      refined_vertex_ids[v] =
          db_->add_vertex(vec2ptr(refined_cell_coordinates) + gd * v);
    } else {
      //    refined_vertex_ids[v] = builder->add_vertex ( vec2ptr (
      //    refined_cell_coordinates ) + gd * v );
    }

    LOG_DEBUG(2, "added vertex " << v << " with id " << refined_vertex_ids[v]
                                 << " and coords "
                                 << refined_cell_coordinates[gd * v] << ", "
                                 << refined_cell_coordinates[gd * v + 1]
                                 << " to DB ");
  }
  ...
  std::vector< AlignNumber > parent_facet_aligns;

  if (td > 1) {
    parent_facet_materials = get_cell_facet_materials(db_, cell);
    parent_facet_aligns = get_cell_facet_aligns(db_, cell);
  }

  // vector for new cell id:s
  std::vector< Id > new_cells(num_children_cells);

  LOG_DEBUG(2, "refined_vertex_ids size = "
                   << refined_vertex_ids.size() << " and content "
                   << string_from_range(refined_vertex_ids.begin(),
                                        refined_vertex_ids.end()));

  int offset = 0;
  for (int rc = 0; rc < num_children_cells; ++rc) {
    // get vertex id:s
    const int cell_size = refined_cell_sizes[rc];
    const std::vector< Id > refined_cell_vertices(
        refined_vertex_ids.begin() + offset,
        refined_vertex_ids.begin() + offset + cell_size);
    offset += cell_size;

    LOG_DEBUG(2, "Refined cell "
                     << rc << " with size " << cell_size << " and offset "
                     << offset - cell_size << " = "
                     << string_from_range(refined_cell_vertices.begin(),
                                          refined_cell_vertices.end()));
    ...
    if (builder == nullptr) {
      db_->set_material_number(td, cell_id,
                               get_material_number(td, cell.index()));
      db_->set_align_number(td, cell_id, get_align_number(td, cell.index()));
    } else {
      //    builder->set_material_number ( td, cell_id, get_material_number (
      //    td, cell.index ( ) ) );
    }
    // TODO builder alternative
    // inherit facet material numbers from parent facets (see helper function
    // above)
    if (!parent_facet_materials.empty()) {
      inherit_facet_material_numbers(
          db_, cell.cell_type(), parent_facet_materials, sub_cell_numbers[rc],
          refined_cell_vertices, tree);
    }
    if (!parent_facet_aligns.empty()) {
      inherit_facet_align_numbers(db_, cell.cell_type(), parent_facet_aligns,
                                  sub_cell_numbers[rc], refined_cell_vertices,
                                  tree);
    }
  }
  return new_cells;
...
MeshPXestBuilder::MeshPXestBuilder(TDim tdim, GDim gdim,
                                   std::vector< MasterSlave > period,
                                   bool is_rectangular)
    : MeshDbViewBuilder(tdim, gdim, 0, period, is_rectangular),
...
void MeshPXestBuilder::add_entity_to_quad_coord(int tdim, Id entity_id,
                                                QuadCoord &coord,
                                                int which_mapping) {
  this->pXest_db_->add_entity_to_quad_coord(tdim, entity_id, coord,
                                            which_mapping);
}

QuadCoord MeshPXestBuilder::get_entity_to_quad_coord(int tdim, Id entity_id,
                                                     int which_mapping) {
  return this->pXest_db_->get_entity_to_quad_coord(tdim, entity_id,
                                                   which_mapping);
}

void MeshPXestBuilder::set_conn_data(int num_vertices, int num_cells,
                                     std::vector< double > &vertices,
                                     std::vector< int > &tree_to_vertices) {
  return this->pXest_db_->set_conn_data(num_vertices, num_cells, vertices,
                                        tree_to_vertices);
}
...
MeshPtr MeshPXestBuilder::build() {
  if (!cells_.empty()) {
    std::vector< bool > locality;
    return MeshPtr(new MeshPXest(this->db_->tdim(), this->db_->gdim(),
                                 this->pXest_db_, this->cells_, 0, locality,
                                 this->period_, this->is_rectangular_));
  }
  return MeshPtr(0);
}
